...
def plot_leis_new_data():
    ...
    for f in ['her_pcCu_lei.csv','her_pcCu_lei_NF.csv','her_pcCu_lei_NF_norm.csv']:
        ...
        if 'NF' in f and 'norm' in f:
            ...
            color='k'
        plt.semilogy(data[:,0],10**(data[:,1]),symbol,color=color,label=label)
...
#        print 'data',f,sp,data[0,1]
        ...
        plt.semilogy([d+0.0592*pH_shift for d in data[:,0]],data[:,1],style,color=color,label=label) #,label=f.replace('calc_std_settings','std')+', '+sp)
        if 'NF' in f:
            plt.semilogy([d+0.0592*pH_shift for d in data[:,0]],[d/SA for d in data[:,1]],next(styles),color='C0',label=label+', normalized') #,label=f.replace('calc_std_settings','std')+', '+sp)
